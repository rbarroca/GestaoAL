...
export default function RegionCard({ name, description, href, available }: RegionCardProps) {
  if (!available) {
    return (
      <div className="block bg-surface-card border border-hairline rounded-xl p-6 opacity-60">
  ...
  return (
    <Link
      to={href}
      className="block bg-surface-card border border-hairline rounded-xl p-6 hover:shadow-card transition-shadow duration-200 group"
    >
